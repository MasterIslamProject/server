...
exports.default = makeTrendingReactQuery;
...
function makeTrendingReactQuery({
  database
}) {
  ...
  async function getTrendingReact({
    max = 100,
    before,
    after
  } = {}) {
    ...
    return (await db.collection('TrendingReact').find(query).limit(Number(max)).toArray()).map(documentToTrendingReact);
  }
  ...
  async function add({
    ...
    return db.collection("TrendingReact").insertOne(trendingReact).then(result => {
  ...
  async function update({
    ...
    const {
      result
    } = await db.collection('TrendingReact').updateOne(query, newSet, {
      upsert: true
    });
  ...
  async function updateMentor({
    ...
    const {
      result
    } = await db.collection('TrendingReact').updateMany(query, newSet);
    return {
      status: "success",
  ...
  async function updateMentorPassword({
    ...
    const {
      result
    } = await db.collection('TrendingReact').updateMany(query, {
      $set: {
        password: bcrypt.hashSync(trendingReact.new_password, 10)
      }
    });
  ...
  async function findByCategory({
    ...
    const db = await database;
    return (await db.collection('TrendingReact').find({
      category: category
    }).toArray()).map(documentToTrendingReact);
  }

  async function findByTrendingId({
    trending_id
  }) {
    const db = await database;
    return (await db.collection('TrendingReact').find({
  ...
  async function deleteByTrendingId({
    trending_id
  }) {
    const db = await database;
    const {
      result
    } = await db.collection('TrendingReact').deleteMany({
      "trending_id": trending_id
    });
    return {
      success: result.n
  ...
  async function deleteById({
    ...
    const {
      result
    } = await db.collection('TrendingReact').deleteOne({
      "_id": db.makeId(id)
    });
  ...
  function documentToTrendingReact({
    _id: id,
    ...doc
  }) {
    return (0, _trendingReact.default)({
      id,
      ...doc
    });
  }
}
//# sourceMappingURL=trending-react-query.js.map