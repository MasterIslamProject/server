...
exports.default = makeReportQuery;
...
function makeReportQuery({
  ...
  async function getReport({
    ...
    return (await db.collection('Reports').find(query).limit(Number(max)).toArray()).map(documentToReport);
  }
  ...
  async function postReport({
    ...
    return db.collection("Reports").insertOne(report).then(result => {
      return {
        message: "Success",
  ...
  async function updateReport({
    ...
    const {
      result
    } = await db.collection('Reports').updateOne(query, newSet, {
      upsert: true
    });
  ...
  async function findById({
    ...
    const db = await database;
    const found = await db.collection('Reports').findOne({
      _id: db.makeId(id)
    });
  ...
  async function findByReporterId({
    reporter_id
  }) {
    const db = await database;
    return (await db.collection('Reports').find({
      reporter_id: reporter_id
    }).toArray()).map(documentToReport);
  }

  async function findByReporteeId({
    reportee_id
  }) {
    const db = await database;
    return (await db.collection('Reports').find({
  ...
  async function findByReportCategory({
    report_category
  }) {
    const db = await database;
    return (await db.collection('Reports').find({
  ...
  async function deleteByReporterId({
    reporter_id
  }) {
    const db = await database;
    const {
  ...
  async function deleteByReporteeId({
    reportee_id
  }) {
    const db = await database;
    const {
