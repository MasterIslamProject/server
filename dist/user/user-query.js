...
function makeUserQuery({
  ...
  async function add({
    ...
    const {
      result,
      ops
    } = await db.collection('Users').insertOne(user).catch(mongoError => {
      ...
      if (found.status == "Suspended") {
        ...
        if (passwordValid) {
          const token = jwt.sign({
            email: email
          }, process.env.JWT_SECRET, {// expiresIn: '900000d'
          });
          return {
            token: token,
  ...
  async function resetPassword({
    ...
    const {
      result
    } = await db.collection('Users').updateOne(query, newSet, {
      upsert: true
    });
